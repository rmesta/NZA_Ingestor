...
tard = ''
date = ''
sysfile = '/tmp/NZA_Ingestor.daemon'
pidfile = sysfile + '.pid'
logfile = sysfile + '.log'
tcpaddr = '127.0.0.1'
tcpport = 12345
...
def Reingest(fpath, cc):
    ...
    # Reingest
    #
    # NZA_Ingestor --reingest { foo.tar.gz | /fully/qualified/path/to/bundle }
    """
    if cc == 'cli':
...
def Ingest(fpath, cc):
    ...
    # Ingest
    #
    # NZA_Ingestor --ingest { foo.tar.gz | /fully/qualified/path/to/bundle }
    """
    if cc == 'cli':
        print_bold('%sing:\t' % whoami(), 'white', False)
        print '\"%s\"' % fpath
...
def Peek(fpath, cc):
    ...
    # Peek
    #
    # NZA_Ingestor --peek { foo.tar.gz | /fully/qualified/path/to/bundle }
    """
    global tard
    global cs_date
    global tcpresp
